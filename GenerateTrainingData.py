...
if __name__ == '__main__':
    textFile = open("labels.txt", "r")
    trueLabelList = []
    for trueLabels in textFile.read().split('\n'):
        trueLabelList.append(trueLabels)
    count = 0
    ...
    for filename in os.listdir('log'):
        if filename[0] != '.':
            row = getData('log/'+filename)
            row.append(trueLabelList[count])
            count += 1
            for rowItem in row:
                trainingData.write(str(rowItem)+',')
            trainingData.write('\n')
            print(row)
    trainingData.close()